...
const MODEL_NAME = "gemini-1.5-flash"; // Or "gemini-2.0-flash" if you prefer
...
const EduSmartApp = () => {
  ...
  const [marksData, setMarksData] = useState(facultyData.marksData);
  ...
  const handleMarksChange = (studentId, field, value) => {
    setMarksData(prev => prev.map(student => 
      student.id === studentId 
        ? { ...student, [field]: value === '' ? 0 : parseInt(value) || 0 }
        : student
    ));
  };

  const calculateTotal = (student) => {
    return (student.ca1 || 0) + (student.ca2 || 0) + (student.ca3 || 0) + 
           (student.midterm || 0) + (student.endterm || 0);
  };
    ...
    <div className="min-h-screen bg-slate-950 text-slate-200 flex overflow-hidden font-sans antialiased">
      ...
      <aside className={`bg-slate-900 border-r border-slate-800 transition-all duration-300 fixed lg:static inset-y-0 left-0 z-40 ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'} w-64 flex flex-col`}>
        ...
        <nav className="flex-1 px-3 mt-6 overflow-y-auto">
          ...
          {userRole === 'student' ? (
            ...
            <>
              ...
              <SidebarBtn icon={FileSpreadsheet} label="Upload Marks" active={activeTab === 'Upload Marks'} onClick={() => handleTabChange('Upload Marks')} />
        ...
        <div className="p-4 border-t border-slate-800">
          <button onClick={handleSignOut} className="w-full flex items-center gap-3 px-4 py-3 rounded-xl text-red-400 hover:bg-red-500/10 transition-all font-bold text-sm">
            <LogOut size={18} />
            <span>Sign Out</span>
          </button>
        </div>
      ...
      <main className="flex-1 overflow-y-auto relative bg-slate-950 flex flex-col lg:ml-0">
              ...
              {showProfileDropdown && (
                <div className="absolute right-0 mt-3 w-56 bg-slate-900 border border-slate-800 rounded-2xl shadow-2xl overflow-hidden animate-in fade-in slide-in-from-top-2 duration-200">
                  <div className="p-4 border-b border-slate-800">
                    <p className="text-sm font-bold text-white">{userRole === 'student' ? studentData.name : facultyData.name}</p>
                    <p className="text-xs text-slate-500 mt-0.5">{userRole === 'student' ? studentData.id : facultyData.id}</p>
                  </div>
                  <div className="p-2">
        ...
        <div className="p-4 lg:p-10 w-full max-w-7xl mx-auto flex flex-col min-h-screen">
          {activeTab === 'Home' ? (
            ...
            <div className="animate-in fade-in slide-in-from-bottom-2 duration-500 w-full flex flex-col">
              ...
              {/* UPLOAD MARKS MODULE - NEW FACULTY FEATURE */}
              {activeTab === 'Upload Marks' && (
                <div className="w-full">
                  <div className="bg-slate-900 border border-slate-800 rounded-[2rem] lg:rounded-[2.5rem] overflow-hidden shadow-2xl">
                    <div className="p-6 lg:p-8 border-b border-slate-800 bg-slate-800/20 flex flex-col md:flex-row md:items-center justify-between gap-4">
                      <div>
                        <h3 className="font-bold text-white text-base lg:text-lg mb-1">Upload Student Marks</h3>
                        <p className="text-xs lg:text-sm text-slate-500">Data Structures & Algorithms • Semester 4</p>
                      </div>
                      <button 
                        onClick={handleSaveMarks}
                        className="bg-indigo-600 hover:bg-indigo-500 text-white text-xs font-black uppercase tracking-widest px-5 lg:px-6 py-2.5 lg:py-3 rounded-xl lg:rounded-2xl transition-all shadow-lg shadow-indigo-600/10 flex items-center gap-2"
                      >
                        <Save size={16} />
                        Save All Marks
                      </button>
                    ...
                    <div className="overflow-x-auto">
                      <table className="w-full text-left border-collapse min-w-[900px]">
                        <thead className="bg-slate-800/50 text-[10px] uppercase font-black tracking-widest text-slate-500 border-b border-slate-800">
                          <tr>
                            <th className="px-6 lg:px-8 py-4 lg:py-5 sticky left-0 bg-slate-800/50 z-10">Student Details</th>
                            <th className="px-4 py-4 lg:py-5 text-center">CA1<br/><span className="text-[8px] text-slate-600">(Max 20)</span></th>
                            <th className="px-4 py-4 lg:py-5 text-center">CA2<br/><span className="text-[8px] text-slate-600">(Max 20)</span></th>
                            <th className="px-4 py-4 lg:py-5 text-center">CA3<br/><span className="text-[8px] text-slate-600">(Max 20)</span></th>
                            <th className="px-4 py-4 lg:py-5 text-center">Midterm<br/><span className="text-[8px] text-slate-600">(Max 50)</span></th>
                            <th className="px-4 py-4 lg:py-5 text-center">Endterm<br/><span className="text-[8px] text-slate-600">(Max 100)</span></th>
                            <th className="px-6 lg:px-8 py-4 lg:py-5 text-center bg-indigo-500/5">Total<br/><span className="text-[8px] text-slate-600">(210)</span></th>
                          </tr>
                        </thead>
                            ...
                            <tr key={student.id} className="hover:bg-slate-800/20 transition-colors">
                              ...
                              <td className="px-4 py-4 lg:py-5 text-center">
                                <input
                                  ...
                                  max="20"
                                  value={student.ca1}
                                  onChange={(e) => handleMarksChange(student.id, 'ca1', e.target.value)}
                                  className="w-16 bg-slate-800/50 border border-slate-700 rounded-lg px-2 py-2 text-center outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500/20 transition-all text-sm font-bold text-white"
                                />
                              </td>
                              <td className="px-4 py-4 lg:py-5 text-center">
                                <input
                                  ...
                                  max="20"
                                  value={student.ca2}
                                  onChange={(e) => handleMarksChange(student.id, 'ca2', e.target.value)}
                                  className="w-16 bg-slate-800/50 border border-slate-700 rounded-lg px-2 py-2 text-center outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500/20 transition-all text-sm font-bold text-white"
                                />
                              </td>
                              <td className="px-4 py-4 lg:py-5 text-center">
                                <input
                                  ...
                                  max="20"
                                  value={student.ca3}
                                  onChange={(e) => handleMarksChange(student.id, 'ca3', e.target.value)}
                                  className="w-16 bg-slate-800/50 border border-slate-700 rounded-lg px-2 py-2 text-center outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500/20 transition-all text-sm font-bold text-white"
                                />
                              </td>
                              <td className="px-4 py-4 lg:py-5 text-center">
                              ...
                              <td className="px-4 py-4 lg:py-5 text-center">
                                <input
                                  ...
                                  onChange={(e) => handleMarksChange(student.id, 'endterm', e.target.value)}
                                  className="w-16 bg-slate-800/50 border border-slate-700 rounded-lg px-2 py-2 text-center outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500/20 transition-all text-sm font-bold text-white"
                                />
                              </td>
                              <td className="px-6 lg:px-8 py-4 lg:py-5 text-center bg-indigo-500/5">
                                <div className="inline-flex items-center justify-center px-4 py-2 bg-indigo-600/20 border border-indigo-500/30 rounded-xl">
                                  <span className="text-lg font-black text-indigo-400">{calculateTotal(student)}</span>
                                </div>
                              </td>
                            </tr>
                          ))}
                    ...
                    <div className="p-6 lg:p-8 border-t border-slate-800 bg-slate-800/10">
                      <div className="flex items-start gap-3">
                        ...
                        <div>
                          <p className="text-sm lg:text-base font-bold text-white mb-1">Auto-Save Enabled</p>
                          <p className="text-xs lg:text-sm text-slate-400 leading-relaxed">Your changes are automatically saved as you type. Click "Save All Marks" to finalize and submit the grades.</p>
                        </div>
                      </div>
                    </div>
                  </div>
        ...
        {/* AI Assistant FAB - FIXED SIZE & POSITIONING */}
        <div className="fixed bottom-4 right-4 lg:bottom-10 lg:right-10 z-50">
          {!showChat ? (
            ...
            <div className="w-96 max-h-[80vh] bg-slate-900 border border-slate-800 rounded-[2rem] shadow-[0_35px_60px_-15px_rgba(0,0,0,0.5)] flex flex-col overflow-hidden animate-in zoom-in-95 duration-300 origin-bottom-right">
              <div className="p-5 bg-indigo-600 flex items-center justify-between text-white shadow-xl relative shrink-0">
                ...
                <div className="flex items-center gap-3 relative z-10">
                  ...
                  <div>
                    <h4 className="font-black text-sm tracking-tight leading-none">EduAI Agent</h4>
                    <p className="text-[10px] text-indigo-100 uppercase tracking-widest font-black mt-1 opacity-80">Online & Ready</p>
                  </div>
                </div>
                <button onClick={() => setShowChat(false)} className="hover:bg-white/10 p-2 rounded-xl transition-all relative z-10">
                  <X size={20} />
                </button>
              </div>
              ...
              <div className="flex-1 overflow-y-auto p-5 space-y-4 bg-slate-950/20">
                ...
                {isTyping && (
                  <div className="flex justify-start">
                    <div className="bg-slate-800 text-slate-500 px-4 py-2.5 rounded-2xl rounded-tl-none text-xs font-bold animate-pulse">
                      Assistant is typing...
                    </div>
                  </div>
                )}
              ...
              <div className="p-4 border-t border-slate-800 bg-slate-900/50 shrink-0">
                <div className="flex items-center gap-3 bg-slate-800 border border-slate-700 rounded-xl px-4 py-2 focus-within:border-indigo-500/50 focus-within:ring-2 focus-within:ring-indigo-500/10 transition-all">
                  <input 
                    type="text" 
                    value={input} 
                    ...
                    className="flex-1 bg-transparent border-none outline-none text-sm py-1.5 text-slate-100" 
                  />
